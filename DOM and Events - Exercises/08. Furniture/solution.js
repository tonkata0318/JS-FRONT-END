function solve() {
  ...
  function parseFurnitureInput() {
    const input = Array.from(JSON.parse(document.querySelector("#exercise textarea").value));
    console.log(input);
    const tableBody = document.querySelector("tbody");
    // TODO: naming
    const cellCreator = createCellCreator();
  
    input.map(cellCreator.createFurnitureRow).forEach((row) => tableBody.appendChild(row));
  }
